...
# legacy, can use numpy or pandas to make it more efficient
"""
def is_na(data):
    data_temp = []
...
def is_na(data):
    return data[~pd.isnull(data)].values

# legacy, use numpy to generate matrix
"""
def date_vec(s):
    w = 6
    h = len(s)
...
def mixed_freq_data(data_y, data_ydate, data_x, data_xdate, x_lag, y_lag, horizon, est_start, est_end, disp_flag = True):
    ...
    data_ydate_num = np.array(data_ydate)
    data_xdate_num = np.array(data_xdate)
    ...
    if x_lag < 0:
        ...
        quit()
    # bug fixed
    min_date_y = data_ydate_num[y_lag]
    ...
    #bug fixed
    tol = timedelta(seconds=0.01)
    loc_start = min(np.where(data_ydate_num >= est_start - tol ))[0]
    loc_end = min(np.where(data_ydate_num >= est_end - tol))[0]
    est_y = data_y[loc_start : loc_end+1]
    est_ydate = data_ydate_num[loc_start : loc_end+1]

    loc_forecast_end = min(np.where(data_ydate_num >= max_date -tol))[0]
    ...
    est_lag_y = np.empty((nobs,y_lag))
    est_lag_ydate = np.empty((nobs,y_lag),dtype=object)
    ...
    if loc_end + 1 <= loc_forecast_end:
        out_lag_y = np.empty((n_forecast,y_lag))
        out_lag_ydate = np.empty((n_forecast,y_lag),dtype=object)
        for m in range(y_lag):
            #fixed bug
            out_lag_y[:,m] = data_y[loc_end - m:loc_forecast_end - m ]
            out_lag_ydate[:,m] = data_ydate_num[loc_end - m:loc_forecast_end - m]
    else:
        out_lag_y = out_lag_ydate = None
    est_x = np.empty((nobs,x_lag))
    est_xdate = np.empty((nobs,x_lag),dtype=object)

    for t in range(nobs):
        #bug fixed
        loc = min(np.where(data_xdate_num >= est_ydate[t] - tol ))[0]
        if loc == None:
            loc = len(data_xdate_num)-1
        if loc - horizon > len(data_x)-1:
            nobs = t - 1
            est_y = est_y[:nobs]
    ...
    if loc_end + 1 <= loc_forecast_end:
        #bug fixed
        out_x = np.empty((n_forecast,x_lag))
        out_xdate = np.empty((n_forecast,x_lag),dtype=object)
        for t in range(n_forecast):
            loc = min(np.where(data_xdate_num >= out_ydate[t] - tol ))[0]
            if loc is None:
                loc = len(data_xdate_num)-1
            if loc - horizon > len(data_x)-1:
                n_forecast = t- 1
                ...
                out_x[t,:] = data_x[loc - horizon : loc - horizon - x_lag: -1]
                out_xdate[t,:] = data_xdate_num[loc - horizon : loc - horizon - x_lag: -1]
    else:
